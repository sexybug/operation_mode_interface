...
int main()
{
    ...
    aes_ccm_updateAData(&ctx, std_A, 7, 0);
    aes_ccm_updateAData(&ctx, std_A + 7, A_len - 7, 1);

    int out_len1, out_len2, out_len3;
    aes_ccm_update(&ctx, std_P, 7, enc_out, &out_len1);
    aes_ccm_update(&ctx, std_P + 7, P_len - 7, enc_out + out_len1, &out_len2);
    aes_ccm_final(&ctx, enc_out + out_len1 + out_len2, &out_len3, enc_Tag);
